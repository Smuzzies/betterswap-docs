        ...
        <div className="mt-16 flex items-center justify-between border-t border-white/5 pt-6">
            {prev ? (
                <Link href={prev.href} className="group flex flex-col gap-0.5">
                    ...
                    <span className="text-sm text-white/60 transition-colors group-hover:text-white">← {prev.label}</span>
                </Link>
            ) : <div />}
            {next ? (
                <Link href={next.href} className="group flex flex-col items-end gap-0.5">
                    <span className="text-xs text-white/30">Next</span>
                    <span className="text-sm text-white/60 transition-colors group-hover:text-white">{next.label} →</span>
                </Link>
            ) : <div />}
        </div>
    );
